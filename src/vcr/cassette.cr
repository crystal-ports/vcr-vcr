...
module VCR
  ...
  class Cassette
    ...
    def initialize(name : String, options : CassetteOptions = CassetteOptions.new)
      @name = name
      @options = CassetteOptions.new
      # Copy default options
      VCR.configuration.default_cassette_options.each { |k, v| @options[k] = v }
      ...
      @tags = @options[:tags]?.try(&.as(Array(Symbol)))
      ...
      @parent_list = @exclusive ? nil : nil # Will be set properly when VCR.http_interactions is available
