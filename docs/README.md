...
The docs come in two flavors:
  ...
  * The [Ruby VCR docs](https://vcr.github.io/vcr) contain example-based documentation which might also be helpful for the Crystal port. It's a good place to look when you are first getting started with VCR.cr, or if you want to see an example of how to use a feature.
...
This is a Crystal port of the original Ruby VCR library. While the API is laaaaaargely similar, there are some Crystal-specific differences.

**Release Policy**

VCR.cr follows the principles of [semantic versioning](https://semver.org/). Patch level releases contain only bug fixes. Minor releases contain backward-compatible new features. Major new releases contain backwards-incompatible changes to the public API.
