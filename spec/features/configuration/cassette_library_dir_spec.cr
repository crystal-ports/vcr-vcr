...
Spectator.describe "Configuration: cassette_library_dir" do
  ...
  describe "directory creation" do
    it "creates the directory if it doesn't exist when recording" do
      ...
      VCR.use_cassette("create_dir_test", options) do
        cassette = VCR.current_cassette
        expect(cassette).not_to be_nil
        if c = cassette
          # Record an interaction to trigger directory creation
          request = VCR::Request.new("get", "http://example.com/test", nil, {} of String => Array(String))
          response = VCR::Response.new(VCR::ResponseStatus.new(200, "OK"), {} of String => Array(String), "test response")
